...
//!
use arthroprod::algebra::{full, ArDifferential, MultiVector, AR};
...
fn energy_momentum_density(f_pq: &MultiVector) {
    let mut em_density: MultiVector = full(f_pq, &f_pq.hermitian());
    em_density.simplify();
...
fn lagrangian_density(f_pq: &MultiVector) {
    let mut l_density: MultiVector = full(f_pq, f_pq);
    l_density.simplify();

    println!("[Lagrangian Density] Fpq ^ Fpq = {}\n", l_density);
}
...
fn stress_tensor(f_pq: &MultiVector) {
    ...
    let mut st: MultiVector = left + right;
    st.simplify();

    println!("[Stress Tensor] (Fpq ^ Fpq!) + (Fpq! ^ Fpq) = {}\n", st);
}
...
fn maxwell(d: &ArDifferential, f_pq: &MultiVector) {
...
/// be pretty messy...)
fn lorentz_force_density(d: &ArDifferential, f_pq: &MultiVector) {
    let m = d.left_apply(&f_pq);
    let mut lf_density: MultiVector = full(f_pq, &m);
    lf_density.simplify();

    println!("[Lorentz Force Density] Fpq Dmu Fpq = {}\n", lf_density);
}
